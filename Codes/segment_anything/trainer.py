...
from Edge_sharpening import SimpleEdgeSobel
...
class SamTrainer:
    ...
    def set_torch_image(
        ...
        if self.edge:
            ...
            # 2. Combine with SAM embedding
            # Assume `embeddings` is from SAM and has shape [B, 256, 64, 64]
            alpha = 0.5
    ...
    def train_model(
        ...
        masks, iou_predictions, low_res_masks = self.train_torch(
            coords_torch,
            labels_torch,
            box_torch,
            mask_input_torch,
...
#         assert 1 == 9
    ...
    def train_torch(
        ...
        if multimask_output:
            best_idx = torch.argmax(iou_predictions_, dim=1).item() 
            iou_predictions = iou_predictions_[0, best_idx].unsqueeze(0) 
            low_res_masks = low_res_masks_[:, best_idx, :, :].unsqueeze(0)
        else:
            iou_predictions = iou_predictions_    
