...
class ConfusionMatrix:
    ...
    def __init__(self, output=None, target=None, device="cuda"):
        self.tp = None
        self.fp = None
        self.tn = None
        self.fn = None
        ...
        # self.target = target
        # self.mask = threshold_mask(output).permute(0,3,1,2)
        # self.output = output
        ...
        if output is not None:
            ...
            self.output = output.float().unsqueeze(1).to(device)  # Ensure it is a float tensor

        self.target = target.unsqueeze(1)
        self.mask = None  # Initialized later based on `output`
        if self.output is not None:
            self.mask = threshold_mask(self.output).permute(0, 3, 1, 2)
        self.reset()
...
def IOU(output=None, target=None, confusion_matrix=None, **kwargs):
    """TN / (TN + FP)"""
